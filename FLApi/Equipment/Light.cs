...
namespace FLApi.Equipment
{
    public class Light : AbstractEquipment
    {
        ...
        public Light(Section section, FreelancerIni freelancerIni)
            : base(section)
        {
                ...
                if (!parentEntry(e))
                {
                    switch (e.Name.ToLowerInvariant())
                    {
                        case "inherit":
                            ...
                            Inherit = freelancerIni.Equipment.FindEquipment(e[0].ToString()) as Light;
                            break;
                        case "always_on":
                            if (e.Count != 1) throw new Exception("Invalid number of values in " + section.Name + " Entry " + e.Name + ": " + e.Count);
                        ...
                        case "glow_color":
                            ...
                            glowColor = new Color(e[0].ToInt32(), e[1].ToInt32(), e[2].ToInt32());
                            break;
                        case "color":
                            if (e.Count != 3) throw new Exception("Invalid number of values in " + section.Name + " Entry " + e.Name + ": " + e.Count);
                            if (color != null) throw new Exception("Duplicate " + e.Name + " Entry in " + section.Name);
                            color = new Color(e[0].ToInt32(), e[1].ToInt32(), e[2].ToInt32());
                            break;
                        case "flare_cone":
                        ...
                        case "min_color":
                            ...
                            minColor = new Color(e[0].ToInt32(), e[1].ToInt32(), e[2].ToInt32());
                            break;
                        case "avg_delay":
                            if (e.Count != 1) throw new Exception("Invalid number of values in " + section.Name + " Entry " + e.Name + ": " + e.Count);
