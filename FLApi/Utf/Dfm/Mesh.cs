...
namespace FLApi.Utf.Dfm
{
    public class Mesh : IDrawable
    {
        ...
        public void Initialize(GraphicsDevice device, ContentManager content, RenderTools.Camera camera)
        {
            ...
            vertexBuffer = new VertexBuffer(device, VertexPositionNormalTexture.VertexDeclaration, vertices.Count, BufferUsage.WriteOnly);
            vertexBuffer.SetData<VertexPositionNormalTexture>(vertices.ToArray());

            foreach (FaceGroup faceGroup in FaceGroups) faceGroup.Initialize(device, content, camera);
        ...
        public void Draw(Color ambient, List<LightSource> lights, Matrix world)
        {
            if (ready)
            {
                ...
                {
                    faceGroup.Draw(vertexBuffer.VertexCount, ambient, lights, world);
                }
            }
        }
    }
