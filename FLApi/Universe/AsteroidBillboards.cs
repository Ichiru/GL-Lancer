            ...
            foreach (Entry e in section)
            {
                switch (e.Name.ToLowerInvariant())
                {
                    case "count":
                        ...
                        if (Count != null) throw new Exception("Duplicate " + e.Name + " Entry in " + section.Name);
                        Count = e[0].ToInt32();
                        break;
                    case "start_dist":
                        if (e.Count != 1) throw new Exception("Invalid number of values in " + section.Name + " Entry " + e.Name + ": " + e.Count);
                        if (StartDist != null) throw new Exception("Duplicate " + e.Name + " Entry in " + section.Name);
                        StartDist = e[0].ToInt32();
                        break;
                    case "fade_dist_percent":
                        if (e.Count != 1) throw new Exception("Invalid number of values in " + section.Name + " Entry " + e.Name + ": " + e.Count);
                        if (FadeDistPercent != null) throw new Exception("Duplicate " + e.Name + " Entry in " + section.Name);
