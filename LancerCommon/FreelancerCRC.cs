...
namespace GLLancer
{
	public static class FreelancerCRC
	{
		//
		private static uint[] modelCrcTable =
		{
		...
		public static uint ModelCRC(string name)
		{
			if (name == null) 
			...
			uint crc = 0xFFFFFFFF;
			for (uint i = 0; i < name.Length; i++) crc = (crc >> 8) ^ modelCrcTable[(byte)crc ^ data[i]];
